Feature: [MLA]  Search PNR information Claim State
  ...
  Scenario: Validate Sale Detail response information from pnr_opened
    Given I want to fetch the information claim state with config "resources/config.json"
    When call "sale-detail" "api.url" more "pnr_opened" with body "resources/request_body.json" and headers "api.headers"
