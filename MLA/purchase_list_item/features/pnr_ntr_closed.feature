Feature: [MLA]  Search PNR information Claim State
  ...
  Scenario: Validate Purchase Detail response information from pnr_ntr_closed
    Given I want to fetch the information claim state with config "resources/config.json"
    When call "purchase-list-item" "api.url" more "pnr_ntr_closed" with body "resources/request_body.json" and headers "api.headers"
