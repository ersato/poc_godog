Feature: [MPE] Search PNR information Claim State
  ...
  Scenario: Validate Sale Detail response information from pnr_reminder
    Given I want to fetch the information claim state with config "resources/config.json"
    When call "sale-detail" "api.url" more "pnr_reminder" with body "resources/request_body.json" and headers "api.headers"
